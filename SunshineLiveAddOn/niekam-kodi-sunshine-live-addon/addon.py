...
jsonData = os.path.join(ptv.getAddonInfo('path'), "resources") + os.path.sep + "config.json"
...
def build_song_list():
    ...
    json_data.close()
    thumbnail_url = data['thumbnail_base_url']
    metadata_url = data['metadata_base_url']
    for song in data["stations"]:
